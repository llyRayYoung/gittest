...
import com.xyd.aike.gank.app.MyApplication;
...
public class StringUtil {
     ...
     *
     * @param str str
     * @return String   首字母为大写的字符串，若首字母为数字或者大写字母直接返回字符串
     */
    ...
    public static void copyToClipboard(String text){
        if(Build.VERSION.SDK_INT >= 11){
            ClipboardManager cbm = (ClipboardManager) MyApplication.getIntstance().getSystemService(Activity.CLIPBOARD_SERVICE);
            cbm.setPrimaryClip(ClipData.newPlainText(MyApplication.getIntstance().getPackageName(), text));
        }else {
            android.text.ClipboardManager cbm = (android.text.ClipboardManager) MyApplication.getIntstance().getSystemService(Activity.CLIPBOARD_SERVICE);
            cbm.setText(text);
        }
    }
